...
def grid_search_C(model, X, y, param_grid, val_curve_path, best_params_path=None):
  # Perform grid search.
  clf = GridSearchCV(model, param_grid, cv=5, n_jobs=-2, iid=False, return_train_score=True)
  clf.fit(X,y)
  C_arr = param_grid[0]['C']
  ...
  np.savetxt(val_curve_path, data.T, header=' C | training score | valid score', fmt='%.0e %.5f %.5f %.5f %.5f')
...
def grid_search_C_and_gamma(model, model_args, X, y, gs_paths):
  C, param_grid = get_C_param_grid()
  ...
  for igamma, gamma_val in enumerate(gamma):
    ...
    grid_search_C(model(**model_args), X, y, param_grid, val_fname)
