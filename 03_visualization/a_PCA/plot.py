...
def load_and_process_data(pseudo=True):
  ...
  y = np.loadtxt(dir_util.clean_features_paths02(istest=not pseudo, pseudo=pseudo).y)
...
def plot(ps_PCA_1, ps_PCA_2, ps_y, PCA_1, PCA_2, y):
  ...
  for latt in cnst.lattices:
    ...
    ax.plot(ps_PCA_1[ps_y==y_lbl], ps_PCA_2[ps_y==y_lbl], latt.ps_pt_fmt, alpha=.3, label='pseudo '+latt.name, mew=0)
