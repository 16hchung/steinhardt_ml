...
def compute(latt, pseudo=.06, cart_only=False):
  # if pseudo, steps will be scaler for random displacement magnitudes
  # if real, use timesteps from real simulation
  steps = np.linspace(.01, pseudo, num=10) if pseudo != None else np.arange(10000,20000+1000,1000) # Step range.
  l = np.arange(1,30+1)
  N_stein = len(l)
  # Iterate over steps.
  X = np.zeros((0,N_stein))
  for step in tqdm(steps):
    ts = 0 if pseudo != None else step
    pipeline = import_file(dir_util.dump_path_for_lattice00(latt, pseudo != None).format(ts))
    data = pipeline.compute()
    if pseudo:
      data = calc.add_offsets(pipeline, data, position_noise_scaler=step)
      export_file(
